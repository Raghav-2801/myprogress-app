        ...
        <div className="px-8 pb-8 max-w-4xl mx-auto">
          <div className="grid grid-cols-3 gap-4 bg-kaleo-cream rounded-2xl p-6 border border-kaleo-terracotta/10">
            <div className="text-center">
              <div className="text-3xl font-bold text-kaleo-brown">{stats.overall.total_questions}</div>
              <div className="text-kaleo-brown/60 text-sm mt-1">Total Questions</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-kaleo-terracotta">{stats.overall.completed_questions}</div>
              <div className="text-kaleo-brown/60 text-sm mt-1">Completed</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-kaleo-brown">{stats.overall.progress_percentage}%</div>
      ...
      <div className="px-8 pb-16 max-w-4xl mx-auto grid gap-4">
        {topics.map((topic, index) => {
          ...
          return (
            ...
            >
              {/* Icon */}
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4" style={{ backgroundColor: topic.color + '20', color: topic.color }}>
                <IconComponent className="w-6 h-6" />
              </div>

              {/* Content */}
              <h3 className="font-playfair text-2xl font-bold text-kaleo-brown mb-2">{topic.name}</h3>
              <p className="text-kaleo-brown/60 text-sm mb-4">{topic.description}</p>

              {/* Progress */}
              <div className="flex items-center gap-2 text-xs text-kaleo-brown/50">
                <span>{topic.completed_count} / {topic.question_count} done</span>
                <span> </span>
